...
<div class="mb-4 row">
    <div class="col-2">
        <div style="width: 10rem; height: 10rem; background-color: #484848; border-radius: 0.8rem; overflow: hidden;">
            <img src="{{ asset($imagePath) }}" alt="" style="width: 100%; height: 100%; object-fit: cover;">
        </div>
    </div>
    ...
    <div class="text-center col-{{ $reservation->status == 'pending' ? '2' : '3' }}">
        ...
        @if ($reservation->status == 'rejected')
            <p>Alasan penolakan: {{  $reservation->reason_reject }}</p>
        @endif
    </div>
    ...
    <div class="text-center col-2">
        ...
        @elseif ($reservation->status == 'rejected')
            <h5 class="gap-2 p-2 text-white rounded d-flex align-items-center justify-content-center"
                ...
                <span class="ti ti-x" style="font-size: 1.8rem;"></span> Verifikasi Ditolak
            </h5>
        @else
            <h5 class="gap-2 p-2 text-white rounded d-flex align-items-center justify-content-center"
        ...
        <div class="text-center col-2">
            <div style="padding-top: 2.8rem;">
                <div style="display: flex; gap: 1rem;">
                    <form class="approve-form" style="display: flex; gap: 1rem;"
                        action="{{ route('admin.reservations.update_status', $reservation) }}" method="Get">
                        <input type="hidden" name="status" value="approved">
                        <button class="ti ti-check approve-button"
                            style=" border: none; padding: 0.6rem; font-size: 2rem; border-radius: 5rem; background-color: #820000; color: #fff;"></button>
                    </form>
                    <form class="reject-form" style="display: flex; gap: 1rem;"
                        action="{{ route('admin.reservations.update_status', $reservation) }}" method="Get">
                        <input type="hidden" name="status" value="rejected">
                        <button class="ti ti-x reject-button"
                            style=" border: none; padding: 0.6rem; font-size: 2rem; border-radius: 5rem; background-color: #820000; color: #fff;"></button>
                    </form>
                </div>
            </div>
