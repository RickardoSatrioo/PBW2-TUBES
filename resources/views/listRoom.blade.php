            ...
            function loadSearchResults(query = '', page = 1, isLoadMore = false) {
                ...
                currentRequest = $.ajax({
                    ...
                    success: function(response) {
                        ...
                        isLoading = false;
                        if (!response.hasMore) {
                            observer.disconnect(); // Stop observing when no more data
                        }
                    },
    ...
    <div style="max-width: 1920px; max-height: 1080px; margin: 0 auto; overflow: hidden; position: relative;">
        ...
        <div style="z-index: 10; width: 100%; position: absolute; height: 5rem; backdrop-filter: blur(2px); background-color: #ffffff88; top: 0; left: 0; padding: 1rem 6rem; display: flex; gap: 6rem; justify-content: space-between; align-items: center;">
            ...
            <div style="flex-grow: 1; display: flex; padding: 1rem; width: 100% align-items: center;">
                <div style="width: 100%; background: #f8f9fa; border-radius: 50px; padding: 5px 10px; display: flex; align-items: center; gap: 15px; box-shadow: 6px 9px 8px rgba(0,0,0,0.1);">
                    <input type="text" id="search" name="seacrh" placeholder="Temukan Ruangan mu di sini" style="width: 100%; padding: 12px; border: none; outline: none; font-size: 1.1rem; background: transparent;">
                    <button class="ti ti-search" id="searchBtn" style="border: none; padding: 0.8rem; font-size: 1rem; border-radius: 50%; background-color: #820000; color: #fff; cursor: pointer; transition: background-color 0.3s ease;"></button>
                </div>
            </div>
            ...
            <div style="display: flex; gap: 1rem; align-items: center;">
                ...
                <button class="ti ti-arrow-narrow-left-dashed" style="font-size: 2.7rem; border: none; background-color: transparent;"></button>
            </div>
        </div>
        ...
        <div style="min-height: 100vh; width: 100%; font-family: Arial, sans-serif; padding-top: 5rem; background: #f8f9fa;">
            ...
            <div style="padding: 0 110px;">
                <div id="room-container" style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 1.5rem; padding: 10px; max-height: 55.6rem; overflow-y: auto;">
